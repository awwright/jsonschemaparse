...
function Schema(id, schema, registry){
	...
	if(schema.const !== undefined){
		...
		if(Array.isArray(schema.const)){
			...
			self.constSchema = new Schema(self.id+'/const', {properties: properties}, self.registry);
...
Schema.prototype.exportRules = function exportRules(){
	...
	if(schema.constValue !== undefined){
		validators.EndNumber.push(function testConst(layer, n){
			if(schema.constValue!==n) return (new ValidationError('const number mismatch', layer, schema, 'const', schema.constLength, n));
		});
	}
	if(schema.enumLiterals){
	...
	if(schema.constValue!==undefined){
		validators.EndBoolean.push(function testTypeBoolean(layer, instance){
			if(schema.constValue!==instance) return (new ValidationError('const mismatch', layer, schema, 'const', schema.constType, schema.constValue));
		});
	}
	...
	if(schema.constValue!==undefined){
		validators.StartNull.push(function testTypeNull(layer, instance){
			if(schema.constType!=='null') return (new ValidationError('const mismatch', layer, schema, 'const', schema.constType, schema.constValue));
		});
	}
...
ValidateLayer.prototype.getAll = function getAll() {
	...
	if(this.const) list.push(this.const);
...
ValidateLayer.prototype.finish = function finish(layer){
	...
	if(self.const) self.const.finish(layer);
	...
	if(schema.constRemain){
		...
		const missing = Object.keys(schema.constValue).filter(function(k){
			return !schema.requiredMap[k];
		});
