...
ValidateLayer.prototype.initProperty = function initProperty(key){
	...
	if(validators.length==0 && schema.unevaluatedProperties){
		// TODO remove the `self` argument here, the validation result will later need to be conditional
		schema.unevaluatedProperties.validate(self).forEach((v) => validators.push(v));
	}
...
ValidateLayer.prototype.startObject = function startObject(layer){
	...
	if(self.schema.required){
		self.requiredRemain = new Set(self.schema.required);
	}
};
