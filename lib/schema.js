...
function Schema(id, schema, registry){
	...
	// "enum"
	// Set of all valid non-string values for an instance
	self.enumValues = null;
	// Optimization for matching strings
	self.enumStrings = null;
	if(schema.enum && Array.isArray(schema.enum)){
		self.enumValues = [];
		self.enumStrings = new Set;
		schema.enum.forEach(function(v){
			if(typeof v==='string' || typeof v==='number' || typeof v==='boolean' || v===null){
				self.enumStrings.add(v);
			}else{
				self.enumValues.push(v);
			}
		});
...
Schema.prototype.exportRules = function exportRules(){
	...
	if(schema.enumStrings){
		validators.EndString.push(function testEnum(layer, str){
			if(!schema.enumStrings.has(str)) return new ValidationError('String does not match one of the enumerated values', layer, schema, 'enum', schema.enumStrings);
		});
	}
	...
	if(schema.enumStrings){
		validators.EndNumber.push(function testEnum(layer, n){
			if(!schema.enumStrings.has(n)) return new ValidationError('Number does not match one of the enumerated values', layer, schema, 'enum', schema.enumStrings);
		});
	}
	...
	if(schema.enumStrings){
		validators.EndBoolean.push(function testEnum(layer, v){
			if(!schema.enumStrings.has(v)) return new ValidationError('Boolean does not match one of the enumerated values', layer, schema, 'enum', schema.enumStrings);
		});
	}
	if(schema.const!==undefined){
	...
	if(schema.enumStrings){
		validators.StartNull.push(function testEnum(layer){
			if(!schema.enumStrings.has(null)) return new ValidationError('Null does not match one of the enumerated values', layer, schema, 'enum', schema.enumStrings);
		});
	}
	if(schema.const!==undefined){
		validators.StartNull.push(function testTypeNull(layer, instance){
			if(schema.constType!=='null') return (new ValidationError('const mismatch', layer, schema, 'const', schema.constType, schema.const));
		});
	}
